    ...
    >
      ...
      <View className="">
        ...
        <Text className="text-gray-700 dark:text-gray-300 mb-2">
          Release: {movie.release_date} | Runtime: {movie.runtime} min | Rating:{" "}
          {movie.vote_average}
        </Text>
        ...
        >
          {movie.production_companies?.map((pc: any) => (
            <View key={pc.id} className="items-center mr-4">
              <Image
                source={{ uri: image185(pc.logo_path) || fallbackMoviePoster }}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {similar.results.slice(0, 10).map((m: any) => (
              <TouchableOpacity
                ...
                className="mr-4 w-36"
                onPress={() =>
                  router.push({ pathname: "/movie/[id]", params: { id: m.id } })
                }
              >
                <Image
                  source={{
